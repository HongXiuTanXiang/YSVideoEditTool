...
- (BOOL)appendWaterMark:(UIImage *)waterImg absoluteRect:(CGRect)absoluteRect {
      if (!waterImg) {
           return NO;
       }
       ...
       runAsynchronouslyOnVideoBoxContextQueue(^{
           ...
           for (WAAVSEComposition *composition in self.workSpace) {
               ...
               [command imageLayerRectWithVideoSize:^CGRect(CGSize videoSize) {
                   ...
                   CGFloat height = 0;
                   if (absoluteRect.size.height) {
                       height = videoSize.height * absoluteRect.size.height;
                   }else{
                       height = videoSize.width * absoluteRect.size.width * waterImg.size.height / waterImg.size.width;
                   }
                   CGFloat x = absoluteRect.origin.x;
                   CGFloat y = absoluteRect.origin.y;
                   return CGRectMake(x, y, videoSize.width * absoluteRect.size.width, height);
               }];
               [command performWithAsset:composition.mutableComposition];
           }
